...
module.exports = function (app, model, io) {
  ...
  app.post('/api/alerts', async (req, res) => {
    ...
    console.log(req.params.msg); // io.sockets.in(req.params.room).emit('alert', ' - emmited..');
  ...
  app.post('/api/alerts/:msg/:room', async (req, res) => {
    ...
    io.sockets.in(req.params.room).emit('alert', req.params.msg); // socket.emit('alert',' - emmited..');
  ...
  app.get('/api/alerts/:alert', async (req, res) => {
    ...
    io.sockets.in(alert.room).emit('alert', alert.msg); // socket.emit('alert',' - emmited..');
