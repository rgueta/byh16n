...
exports.pwdRSTReq = exports.pwdRSTConfirm = exports.pwdRSTApply = void 0;

var _Users = _interopRequireDefault(require("../models/Users"));

var _config = _interopRequireDefault(require("../config"));

var _pwdRST = _interopRequireDefault(require("../models/pwdRST"));
...
let html_call = ''; // #endregion html layout ---------------------------------------

const pwdRSTReq = async (req, res) => {
  console.log('pwdRSTReq params ---> ', req.params);
  let foundUser = await _Users.default.find({
    email: req.params.email
  });

  try {
    console.log('foundUser ---->  ', foundUser);
    if (!foundUser.length) return res.status(400).json({
      'message': 'User not found'
    }); // #region save on DB   ------------------------------

    const newPwdRST = new _pwdRST.default({
      email: req.params.email,
      confirmed: false,
      reseted: false
    });
    newPwdRST.save(async (err, pwdRST_saved) => {
      if (err) return res.status(400).json({
        'message': 'Error can not saved pwdRST'
      });
    ...
    res.status('201').json({
      'message': 'email sent , hash --> ' + hash_pwdRST
    });
  } catch (e) {
    console.log('error pwdRSTReq -----> ', e);
    res.status('401').json({
      'pwdRSTReq error': e.message
    });
  }
};

exports.pwdRSTReq = pwdRSTReq;

const pwdRSTConfirm = async (req, res) => {
  const foundpwdRST = await _pwdRST.default.find({
    _id: req.params.id
  });
  if (!foundpwdRST.length) return res.status(400).json({
    'error': 'No hay solicitud para este usuario'
  });
  const foundUser = await _Users.default.findOne({
    email: foundpwdRST[0].email
  });
  if (!foundUser) return res.status(400).json({
    'error': 'User not found'
  });

  _pwdRST.default.findByIdAndUpdate(req.params.id, {
    $set: {
      confirmed: true
    }
  }, {
    ...
    if (err) {
      return res.status(400).json({
        'error': 'Could not update reseted.'
      });
    }
  });

  res.status(201).json({
    'msg': 'Password reset confirmation for [ ' + foundUser.email + ' ]'
  });
};

exports.pwdRSTConfirm = pwdRSTConfirm;

const pwdRSTApply = async (req, res) => {
  ...
  try {
    ...
    if (!Encryptedpwd) return res.status(400).json({
      'error': 'No password encrypted.'
    });
    ...
    _Users.default.findOneAndUpdate({
      ...
      res.status(201).json({
        'msg': 'Password changed for [' + foundUser.email + '], encrypted pwd --> ' + Encryptedpwd
      });
...
exports.pwdRSTApply = pwdRSTApply;
  ...
  html_call = `
    ...
    <html>
        ...
        <body>
            <div class="head">
                <div class="logo">
                    <img src="${_config.default.app.public_host}/img/logo_v2.png">
                </div>
            </div>
            ...
            <a href='${_config.default.app.public_host}/pwdResetReq?req=${pwdRST_id}' target='#'>
...
//# sourceMappingURL=pwdRST.controller.js.map