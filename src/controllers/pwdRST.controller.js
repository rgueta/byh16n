...
export const pwdRSTReq = async (req, res) => {
  ...
  try {
    ...
    const foundPwdRST = await pwdRST.find({
    ...
    if (foundPwdRST.length > 0) {
      return res
        .status(503)
        .json({ status: 503, msg: "Ya existe peticion para este correo" });
    }

    const newPwdRST = new pwdRST({
    ...
    const smtpCnnection = transporter.sendMail(mailOption);
    return res.status(201).json({ "email sent": smtpCnnection.response });
...
export const pwdRSTConfirm = async (req, res) => {
  try {
    ...
    if (!foundpwdRST.length)
      return res
        .status(300)
        .json({ status: 300, error: "No hay solicitud para este usuario" });

    const foundUser = await Users.findOne({ email: foundpwdRST[0].email });
    if (!foundUser)
      return res.status(300).json({ status: 300, error: "User not found" });

    pwdRST.findByIdAndUpdate(
      req.params.id,
      { $set: { confirmed: true } },
      { new: false },
      (err, resultConfirmed) => {
...
export const pwdRSTVerify = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ status: 200, msg: "Si hay solicitud activa" });
  } catch (e) {
...
export const pwdRSTApply = async (req, res) => {
  ...
  try {
    const foundUser = await Users.findOne({ email: foundpwdRST[0].email });
    if (!foundUser)
      return res.status(301).json({ status: 301, error: "User not found" });

    const Encryptedpwd = await encryptPassword(req.params.pwd);
    if (!Encryptedpwd)
      return res
        .status(302)
        .json({ status: 302, error: "No password encrypted." });

    Users.findOneAndUpdate(
      { email: foundpwdRST[0].email },
      { $set: { pwd: Encryptedpwd } },
      { new: false },
      (err, result) => {
        if (err) {
          return res
            .status(500)
            .json({ status: 500, error: "Could not update password." });
        }
        ...
        pwdRST.findByIdAndUpdate(
          ...
          (err, resultReseted) => {
            if (err) {
              return res
                .status(501)
                .json({ status: 501, error: "Could not update reseted." });
            }
          },
        );
