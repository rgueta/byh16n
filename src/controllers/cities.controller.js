...
export const getCities = async (req, res) => {
    await Cities.find({country:req.params.country,state:req.params.state}, (err, results) => {
        if (err) {
          console.log(err);
...
export const deleteCityById = async (req,res) => {
  const deletedCity = await Cities.findByIdAndDelete(req.params.countryId,
    req.params.stateId,req.params.cityId);
