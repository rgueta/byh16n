    ...
    try {
      ...
      new Upload({
        ...
        if (err) {
          ...
          const webImgRoot =
            process.env.AWS_BUCKET_NAME + locationFolder + "/" + folder + "/";
          const location = locationFolder;
          const size = req.file.size;
          await tools
            .getSection(data.Location, "path")
            .then(async (section, fail) => {
              if (fail) {
              ...
              const newInfo = await information({
                title,
                url,
                description,
                image,
...
export const getInfo = async (req, res) => {
  let now = new Date(req.params.now);
...
export const getInfoAdmin = async (req, res) => {
  try {
    await information
      ...
      .catch((err) => {
        res.status(502).json({ "Error getting info --> ": err });
      });
  } catch (err) {
    res.status(502).json({ Error: "Error getting info" + err });
  }
