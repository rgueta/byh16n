...
export const createCode_event = async (req, res) => {
  try {
    ...
    const codeId = new Types.ObjectId(String(code_Id));
...
export const getCode_events = async (req, res) => {
  ...
  console.log("getCode_events - parameters --> ", req.params);

  if (req.params.userId === undefined) {
    console.log("getCode_events - userId not received on server --> ");
    res.status(303).json({ msg: "userId not received on server" });
    return;
  }
