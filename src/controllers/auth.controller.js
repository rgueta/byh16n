    ...
    await Users.aggregate([
      ...
      },
      {
        $lookup: {
          ...
          as: "config",
          pipeline: [
            { $match: { _id: new Types.ObjectId("65a822555b9c9da318f78179") } },
          ],
        },
      },
      { $unwind: "$config" },
      ...
      .then(async (foundUser) => {
        const matchPwd = await Users.comparePassword(
          req.body.pwd,
          foundUser[0].pwd,
        );
        if (!matchPwd)
          return res
            .status(400)
            .json({ token: "", ErrMsg: "Invalid password" });
        // Create tokens  ------------------------
        jwt.sign(
          { id: foundUser[0]._id },
          process.env.SECRET,
          ...
          async (err, token) => {
            ...
            console.log(
              "email: " +
                ...
                req.body.pwd +
                ", encrypted: " +
                (await Users.encryptPassword(req.body.pwd)),
            );
            ...
            let iatDate = new Date(decode.iat * 1000);

            expDate.setMinutes(
              expDate.getMinutes() - expDate.getTimezoneOffset(),
            );
            iatDate.setMinutes(
              iatDate.getMinutes() - iatDate.getTimezoneOffset(),
            );
            ...
            if (!token)
              return res
                .status(401)
                .json({ token: "", message: "Something goes wrong" });
            ...
            return res.status(201).json({
              ...
              sim: foundUser[0].sim,
              coreSim: foundUser[0].coreSim,
              pwd: foundUser[0].pwd,
              locked: foundUser[0].locked,
              coreId: foundUser[0].core,
              coreName: foundUser[0].coreName,
              email: foundUser[0].email,
              location: foundUser[0].location,
              backendUrl: foundUser[0].backendUrl,
              localUrl: foundUser[0].localUrl,
              code_expiry: foundUser[0].code_expiry,
              iatDate: iatDate,
              expDate: expDate,
      ...
      .catch((err) => {
        ...
        return res
          .status(400)
          .json({ errId: 1, ErrMsg: "Usuario no encontrado", Error: err });
      });
  } catch (err) {
    console.log("Error Auth: ", err);
          ...
          if (err) {
            ...
            await jwt.sign(
              ...
              async (err, refreshToken) => {
                if (err)
                  return res.state(400).json({ msg: "new refresh faild" });
          ...
          await jwt.sign(
            ...
            async (err, token) => {
              ...
              if (err) {
                return res
                  .status(400)
                  .json({ "Something goes wrong": err.message });
              }
              ...
              await expDate.setMinutes(
                expDate.getMinutes() - expDate.getTimezoneOffset(),
              );
              await iatDate.setMinutes(
                iatDate.getMinutes() - iatDate.getTimezoneOffset(),
              );
