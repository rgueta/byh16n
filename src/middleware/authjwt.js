...
export const isNeighbor = async (req, res, next) => {
  ...
  try {
    ...
    if (req.params.userId) {
      if (!founduser)
        return res.status(400).json({ error: "isNeighbor user not found" });
    } else if (req.params.email) {
      if (!founduser)
        return res.status(405).json({ status: "405", msg: "email not found" });
    }

    // user FOUND -----------------
    if (req.params.email) {
      //verifying for PwdRST  -------------------
      ...
      if (!found_roles)
        return res.status(400).json({ error: "roles not found for user" });

      let neighbor_role = found_roles.find(
        (el) => el.name == "neighbor" || el.name == "neighborAdmin",
      );
